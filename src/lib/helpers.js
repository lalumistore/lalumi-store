...
// ---- Status do pedido (ciclo completo, pagamento até entrega) ----
...
export const ORDER_STATUS_COLOR = {
  aguardando_pagamento: { bg: "#F2EEE1", text: "#6B7A7A" },
  pagamento_confirmado: { bg: "#E5F7F7", text: BRAND.tealDark },
  aguardando_envio: { bg: "#FFF3D6", text: "#8A6D1D" },
  enviado: { bg: "#E5F7F7", text: BRAND.tealDark },
  saiu_para_entrega: { bg: "#FBEAF0", text: "#993556" },
  entregue: { bg: "#E9F5DD", text: "#5C7A2B" },
  cancelado: { bg: "#FCE8E6", text: "#B23A2F" },
};
// Ordem das etapas normais do pedido, pra desenhar a barra de progresso
// (cancelado fica fora dessa linha, é tratado à parte).
export const ORDER_PROGRESS_STEPS = ["pagamento_confirmado", "aguardando_envio", "enviado", "saiu_para_entrega", "entregue"];


// ---- CEP e frete ----
// Busca endereço a partir do CEP usando o ViaCEP (gratuito, sem chave).
...
// Base simples de cálculo de frete por região, pra já ter algo
// funcionando. O ideal, mais adiante, é trocar por uma integração de
// verdade (Correios ou Melhor Envio), que calcula com base no peso e
// na distância exata.
const NORTHEAST_UFS = ["PE", "PB", "AL", "RN", "CE", "SE", "BA", "PI", "MA"];
export function estimateShipping(uf) {
  if (!uf) return 0;
  if (uf === "PE") return 12.9;
  if (NORTHEAST_UFS.includes(uf)) return 19.9;
  return 29.9;
}
