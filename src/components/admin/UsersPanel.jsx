...
import { BRAND, formatPrice, formatDate, ORDER_STATUS_LABEL, ORDER_STATUS_COLOR } from "../../lib/helpers.js";
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {orders && orders.map((o) => {
                ...
                return (
                  <div key={o.id} className="rounded-xl p-2.5" style={{ border: `2px solid ${o.stock_issue ? "#F5D0CB" : "#EFEADC"}` }}>
                    <div className="flex justify-between items-center gap-2">
                      <span className="text-sm font-bold">#{o.order_number}</span>
                      <span className="text-xs font-bold rounded-full px-2 py-0.5 flex-shrink-0" style={{ background: color.bg, color: color.text }}>{ORDER_STATUS_LABEL[o.status] || o.status}</span>
                      <span className="font-bold text-sm flex-shrink-0" style={{ color: BRAND.tealDark }}>{formatPrice(total(o))}</span>
                    </div>
                    <div className="text-xs opacity-50 mt-1">{new Date(o.created_at).toLocaleDateString("pt-BR")} · {o.order_items.map((i) => `${i.quantity}x ${i.product_name}`).join(", ")}</div>
                    {o.stock_issue && (
                      <div className="mt-1.5 text-xs font-bold rounded-lg px-2 py-1" style={{ background: "#FCE8E6", color: "#B23A2F" }}>⚠️ Estoque insuficiente ao confirmar</div>
                    )}
                  </div>
                );
              })}
