...
import {
  ...
  maskCPF, maskPhone, effectiveDiscountPct, effectivePrice, todayISO, iconForCategory,
} from "./lib/helpers.js";
...
const emptyCheckoutForm = { nome: "", email: "", celular: "", cpf: "", rua: "", numero: "", bairro: "", cidade: "", cep: "", estado: "" };
...
export default function App() {
  ...
  const cartHasStockIssue = cartItems.some((i) => i.qty > (i.size_stock?.[i.size] || 0));
  ...
  const buildSupportMessage = () =>
    `Olá! Preciso de ajuda com meu pedido na Lalumi Store${createdOrder ? ` (nº ${createdOrder.order_number})` : ""}.`;
  ...
  const submitOrder = async (e) => {
    ...
    if (!form.estado.trim()) return setCheckoutError("Informe o estado (UF) do endereço.");
    if (cartItems.length === 0) return setCheckoutError("Seu carrinho está vazio.");
    if (cartHasStockIssue) return setCheckoutError("Algum item do carrinho ficou esgotado ou com menos estoque do que a quantidade escolhida. Volte no carrinho e ajuste antes de continuar.");
    setCheckoutError("");
    ...
    try {
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          address_state: form.estado.trim().toUpperCase(),
      ...
      // Salva o endereço no cadastro, se a pessoa marcou a opção (não bloqueia o pedido se falhar)
      addressStepRef.current?.saveThisAddress().catch(() => {});
      ...
      // Importante: o estoque só é debitado quando o pagamento é
      // aprovado de verdade — isso acontece no webhook do Mercado
      // Pago (mercadopago-webhook.js), não aqui. Assim, um pedido que
      // nunca chega a ser pago não mexe no estoque nem na auditoria.
        ...
        <div className="fixed inset-0 z-30 flex justify-end" style={{ background: "rgba(30,58,58,0.4)" }} onClick={() => setCartOpen(false)}>
          <div className="w-full max-w-sm h-full flex flex-col" style={{ background: BRAND.cream }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              ...
              {cartItems.map((i) => {
                const available = i.size_stock?.[i.size] || 0;
                const soldOut = available <= 0;
                const insufficient = !soldOut && i.qty > available;
                return (
                  <div key={i.cartKey} className="rounded-2xl p-3 flex gap-3 items-center" style={{ background: "#fff", border: `2px solid ${soldOut || insufficient ? "#F5D0CB" : "#EFEADC"}` }}>
                    <div style={{ width: 56, height: 56, opacity: soldOut ? 0.5 : 1 }}><ProductThumb product={i} size="56px" /></div>
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        <button onClick={() => changeQty(i.cartKey, 1)} disabled={i.qty >= available} className="rounded-full p-1 disabled:opacity-30" style={{ background: "#F2EEE1" }}><Plus size={12} /></button>
                      </div>
                      {soldOut ? (
                        <div className="text-xs font-bold mt-1" style={{ color: "#B23A2F" }}>Esgotado — remova este item pra continuar</div>
                      ) : insufficient ? (
                        <div className="text-xs font-bold mt-1" style={{ color: "#B23A2F" }}>Só restam {available} — ajuste a quantidade</div>
                      ) : (
                        <div className="text-xs opacity-40 mt-1">{available} disponíveis</div>
                      )}
                    </div>
            ...
            {cartItems.length > 0 && (
              <div className="p-4" style={{ borderTop: "2px solid #EFEADC" }}>
                {cartHasStockIssue && (
                  <div className="text-xs font-bold rounded-lg px-3 py-2 mb-3" style={{ background: "#FCE8E6", color: "#B23A2F" }}>
                    Ajuste ou remova os itens esgotados/indisponíveis pra continuar.
                  </div>
                )}
                ...
                <button onClick={openCheckout} disabled={cartHasStockIssue} className="w-full rounded-full py-3 font-bold disabled:opacity-40" style={{ background: BRAND.pink, color: "#fff" }}>Finalizar compra</button>
              </div>
            )}
          </div>
        </div>
            ...
            {!createdOrder ? (
              <>
                ...
                <div className="p-5 flex flex-col gap-3 max-h-[70vh] overflow-y-auto">
                  ...
                  <div>
                    <label className="text-xs font-bold opacity-60 flex items-center gap-1 mb-1.5"><MapPin size={13} className="opacity-60" /> Endereço de entrega</label>
                    <AddressStep ref={addressStepRef} userId={session?.user?.id} form={form} setForm={setForm} onShippingChange={setShippingCost} />
                  </div>
                  <div className="rounded-xl px-3 py-2 mt-1 flex flex-col gap-1" style={{ background: "#F2EEE1" }}>
                    <div className="flex justify-between text-sm">
                      <span className="opacity-70">Subtotal</span>
                      <span>{formatPrice(cartTotal)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="opacity-70">Frete estimado</span>
                      <span>{shippingCost > 0 ? formatPrice(shippingCost) : "Informe o CEP"}</span>
                    </div>
                    <div className="flex justify-between font-bold pt-1" style={{ borderTop: "1px solid #E3DED0" }}>
                      <span className="text-sm">Total do pedido</span>
                      <span style={{ color: BRAND.tealDark }}>{formatPrice(grandTotal)}</span>
                    </div>
                  </div>
              ...
              <>
                ...
                <div className="p-5 max-h-[75vh] overflow-y-auto">
                  <div className="flex justify-between items-center rounded-xl px-3 py-2 mb-3" style={{ background: "#F2EEE1" }}>
                    <span className="text-sm font-bold">Total a pagar (com frete)</span>
                    <span className="font-bold" style={{ color: BRAND.tealDark }}>{formatPrice(createdOrder.total)}</span>
                  </div>
                  <PaymentBrick
              ...
              <div className="p-8 text-center flex flex-col items-center gap-3">
                {paymentResult.ok && paymentResult.status === "approved" ? (
                  <>
                    ...
                    <p className="text-sm opacity-60">Seu pedido nº {createdOrder.order_number} foi confirmado. Acompanhe tudo em "Meus pedidos".</p>
                  </>
